...
/************************************************/	
	...
	//Construct gallery panels from '.panel' <div>s
		function buildPanels() {
			...
			if(!opts.show_filmstrip) {
				...
				$('<img />').addClass('nav-next').attr('src','images/next.gif').appendTo(j_gallery).css({
					'position':'absolute',
					'zIndex':'1100',
					'cursor':'pointer',
				...
				$('<img />').addClass('nav-prev').attr('src','images/prev.gif').appendTo(j_gallery).css({
					'position':'absolute',
					'zIndex':'1100',
					'cursor':'pointer',
					'top':((opts.panel_height-22)/2)+gallery_padding+'px',
			...
			if(scale_panel_images) {
				$('img',j_panels).each(function(i){
					$(this).css({
						'max-height':panel_img_scale[i%item_count]*img_h[i%item_count],
						'max-width':panel_img_scale[i%item_count]*img_w[i%item_count],
						'position':'relative',
	...
	//Construct filmstrip from '.filmstrip' <ul>
		function buildFilmstrip() {
			...
			//Add navigation buttons
			var navNext = $('<img />');
			navNext.addClass('nav-next').attr('src','images/next.gif').appendTo(j_gallery).css({
				'position':'absolute',
				'cursor':'pointer'
			}).click(showNextItem);
			var navPrev = $('<img />');
			navPrev.addClass('nav-prev').attr('src','images/prev.gif').appendTo(j_gallery).css({
				'position':'absolute',
				'cursor':'pointer'
			}).click(showPrevItem);
			if(filmstrip_orientation=='horizontal') {
